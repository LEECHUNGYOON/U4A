...
sap.ui.define([
	...
	CalendarRow.prototype.onsapselect = function(oEvent){
		...
		//[U4A-001] 20221104 PES -START. (v1.107.1, v1.120.21)
		//CalendarRow의 intervalHeaders aggregations에 존재하는 UI를 선택하는경우 발생하는 오류에 대한 로직 보완 조치.
		//(Appointment aggregations에 존재하는 UI를 선택할때만 sBundleKey를 구성하기에 발생된 문제)
		// this._oInvisibleMessage.announce(this._oRb.getText(sBundleKey), InvisibleMessageMode.Polite);

		//sBundleKey 이 존재하는 경우에만 로직 처리 내용 추가.
		if(sBundleKey){this._oInvisibleMessage.announce(this._oRb.getText(sBundleKey), InvisibleMessageMode.Polite);}
		//[U4A-001] 20221104 PES -END. (v1.107.1, v1.120.21)
	...
	*/
	...
	function _selectAppointment(oAppointment, bRemoveOldSelection) {
		...
		//[U4A-002] 20251014 PES -START. (v1.120.21)
		//Appointment DOM에 aria-labelledby 가 설정되어 있지 않기에, 오류가 발생되고 있음.
		//이에 따른 기존 로직 주석처리.
		...
		//1.120.22 버전에 해당 버그가 수정되어 있는 로직을 적용 처리.
		...
		//[U4A-002] 20251014 PES -END. (v1.120.21)
